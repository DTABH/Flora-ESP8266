...
void IRAM_ATTR  TimerHandler()
{
  ...
  if (enableDotsAnimation) {
	...
	{
		#if defined(CLOCK_VERSION_IV12)
		segmentBrightness[1][4] = bri_vals_separate[bri][1];
		segmentBrightness[3][4] = bri_vals_separate[bri][3];
    #elif defined(CLOCK_VERSION_IV11_RV)
 	  segmentBrightness[0][7] = bri_vals_separate[bri][0];
		segmentBrightness[2][7] = bri_vals_separate[bri][2];    
		#else
		segmentBrightness[1][7] = bri_vals_separate[bri][1];
	  segmentBrightness[3][7] = bri_vals_separate[bri][3];
		#endif
	}	
...
void handleFade() 
{
  for (int i = 0; i < registersCount; i++) 
  {
    for (int ii = 0; ii < segmentCount; ii++) 
    {
      if (targetBrightness[i][ii] > bri_vals_separate[bri][i]) 
      {
        targetBrightness[i][ii] = bri_vals_separate[bri][i];
      }
...
{
  #if defined(CLOCK_VERSION_IV11_RV)
  digit =  registersCount -1 - digit;
  #endif
...
void toggleNightMode() 
{
  ...
  if (nmode > 0 && (hour() >= (nmode + 18)||( hour() <= (nmodeoff + 5)))) {
    Serial.println("Nightmode activ hour(): " + String(hour()));
    bri = 0;
    colon = 0;
  } else {
    bri = json["bri"].as<int>();
